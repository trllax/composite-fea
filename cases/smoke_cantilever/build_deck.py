...
# 32 along the length is where this load case converges and 1 across the width is
# enough -- both measured, see README.md. The mesh comes out of geometry.py
# rather than a checked-in file so that this case gates the mesher too, which is
# what CLAUDE.md already says it does.
N_CHORD = 1
N_SPAN = 32
...
@lru_cache(maxsize=1)
def mesh_inp() -> str:
    """The strip mesh, built by geometry.py. Cached: gmsh runs are not free."""
    return mesh_outline(
        Outline.rectangle(chord=WIDTH_MM, span=LENGTH_MM),
        n_chord=N_CHORD,
        n_span=N_SPAN,
        heading=(
            f"smoke_cantilever mesh: {LENGTH_MM:g} x {WIDTH_MM:g} mm strip, "
            f"{N_SPAN} S8R elements along the length, {N_CHORD} across.\n"
            "x = width, y = length (the long axis), z = shell normal (+z).\n"
            "Generated by compfea.geometry -- do not check a copy of this in."
        ),
    ).to_inp()
