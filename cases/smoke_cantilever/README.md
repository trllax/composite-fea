...
(`mesh_strip_32el.inp`). Axes: x = width, **y = the long axis**, z = normal, so
`long_axis = "y"` and a 0-degree ply runs along the beam.
...
| file | what it is |
| --- | --- |
| `mesh_strip_32el.inp` | nodes, elements, node sets. Hand-generated; `geometry.py` will emit it |
