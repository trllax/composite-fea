#!/usr/bin/env python3
"""Tip U-clamp on plybook-built FIN_TEST_3 (span +y, HEAL clamp, tip at ymin)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--plybook",
        type=Path,
        default=ROOT / "cases/fin_test_3/plybook_shop.csv",
    )
    p.add_argument(
        "--materials",
        type=Path,
    ...
    p.add_argument("--end-deg", type=float, default=5.0)
    ...
    p.add_argument(
        "--run-dir",
        type=Path,
        default=ROOT / "results" / "fin_test_3_ubend",
    )
    ...
    deck = build_deck(
        ...
        heading=(
            f"FIN_TEST_3 U-bend plybook={args.plybook.name} "
    ...
    for deg in sorted({args.end_deg, 90.0, 180.0}):
        if deg > args.end_deg + 1e-9 or deg < args.start_deg - 1e-9:
            continue
