...
def test_shipped_shop_matches_im_cards_and_keeps_fibre_out_of_e1():
    ...
    # thickness_mm is the as-laid cured value = gsm/1000 * CURED_PLY_FACTOR (1.1).
    im2 = skus["hexcel_im2_uni_193"]
    assert im2.architecture == "ud" and im2.thickness_mm == pytest.approx(0.2123)
    ...
    assert biax.thickness_mm == pytest.approx(0.11)
    assert biax.record.constants.e1 == pytest.approx(59160.0)
    im7 = skus["hexcel_im7_twill_205"]
    assert im7.architecture == "0_90" and im7.thickness_mm == pytest.approx(0.2255)
    ...
    assert ud.thickness_mm == pytest.approx(0.4169)
...
def test_oz_only_converts_to_gsm(tmp_path):
    ...
    assert sku.thickness_mm == pytest.approx(sku.areal_weight_gsm / 1000 * 1.1)
